...
def train(args):
    ...
    sents = []
    for s,sc in zip(df["Text"].tolist(), df["Score"].tolist()):
        sents.append(InputExample(texts=s.split("\n"), label=sc))
    word_embedding_model = models.Transformer(model_name)
    pooling_model = models.Pooling(word_embedding_model.get_word_embedding_dimension(), 'cls')
    model = SentenceTransformer(modules=[word_embedding_model, pooling_model])  
    
    train_dataloader = DataLoader(sents, batch_size=64, shuffle=True)
...
if __name__=='__main__':
    ...
    parser.add_argument('-s', '--out_file', default="./pred_eng_a.csv", type=str, 
